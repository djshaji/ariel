...
static void
on_remove_all_clicked(GtkButton *button, ArielWindow *window)
{
    if (!window) {
    ...
    g_print("Removed all active plugins\n");
}

// Callback for individual plugin remove button
static void
on_remove_plugin_clicked(GtkButton *button, ArielWindow *window)
{
    ArielActivePlugin *plugin = g_object_get_data(G_OBJECT(button), "plugin");
    if (!plugin) {
        g_warning("No plugin associated with remove button");
        return;
    }
    
    if (!window) {
        g_warning("No window reference for remove button");
        return;
    }
    
    // Get plugin manager and remove the specific plugin
    ArielPluginManager *manager = ariel_app_get_plugin_manager(window->app);
    if (!manager || !manager->active_plugin_store) {
        return;
    }
...
ariel_create_active_plugins_view(ArielWindow *window)
{
    ...
    gtk_box_set_vexpand(GTK_BOX(plugins_box), TRUE);
    gtk_box_set_hexpand(GTK_BOX(plugins_box), TRUE);
...
ariel_create_active_plugin_widget(ArielActivePlugin *plugin, ArielWindow *window)
{
    ...
    // Store plugin reference with the button for removal callback
    g_object_set_data(G_OBJECT(remove_btn), "plugin", plugin);
    ...
    GtkWidget *param_controls = ariel_create_parameter_controls(plugin);
    if (param_controls) {
        gtk_box_append(GTK_BOX(main_box), param_controls);
    }
...
{
    if (!window->active_plugins) return;
    
    // Get plugins container
    GtkWidget *plugins_box = g_object_get_data(G_OBJECT(window->active_plugins), "plugins-box");
